...
# API base 는 API GW 직접 URL 을 default 로 사용 — api.aiseo.tips 커스텀
# 도메인이 API GW 에 연결돼 있지 않아 SPA fallback 으로 빠지기 때문.
# (custom domain 셋업 후엔 VITE_API_BASE_URL_PROD 로 override)
_API_BASE="${VITE_API_BASE_URL_PROD:-https://1ni4szkrqh.execute-api.ap-northeast-2.amazonaws.com/prod}"
sed -e "s|%%COGNITO_REGION%%|${_REGION}|g" \
    -e "s|%%COGNITO_CLIENT_ID%%|${_CLIENT_ID}|g" \
