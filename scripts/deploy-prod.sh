...
# 3. B2B 페이지 alias 업로드
# b2b.aiseo.tips 는 CF subdomain-router 가 모든 비-자산 경로를
# /site/b2b/index.html 로 보낸다. 그 객체는 4단계 dist/→/site/ sync 가
# frontend/public/b2b/index.html(자급식 정적 랜딩, Vite 가 dist/b2b/index.html
# 로 복사)을 업로드하면서 채운다. 아래 두 줄은 apex 단축링크(aiseo.tips/b2b.html)
# 와 레거시 /b2b/index.html alias 유지용.
echo "[3/5] Uploading B2B page aliases..."
aws s3 cp frontend/dist/b2b/index.html s3://$BUCKET/b2b.html \
  --content-type "text/html; charset=utf-8" --profile $PROFILE
aws s3 cp frontend/dist/b2b/index.html s3://$BUCKET/b2b/index.html \
  --content-type "text/html; charset=utf-8" --profile $PROFILE
