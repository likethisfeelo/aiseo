...
/**
 * 첫완성패키지 (풀패키지) — `/events2026/first`
 *
 * EVENT 04. 단일 풀패키지 상세 페이지로, 검색 전략(001) + 콘텐츠
 * 기획(002) + 즉시 배포(CORE1) 전 과정에 사전상담 1시간을 더해
 * 정가 50만원 → 35만원으로 진행한다. 신청은 모두 `EVENT_04_PAID`.
 *
 * 구조는 `/events2026/paid`(Events2026PaidPage)를 복제해 단일 패키지로
 * 적응한 것 — HERO, 포함 과정 미리보기, 풀패키지 상세(가격 박스 +
 * 001/002/CORE1/사전상담 core-card), 공통 대상, FAQ, "왜 풀패키지" 비교,
 * 카운트다운 LAUNCH CTA, FREE 크로스링크.
 *
 * Class naming: every selector specific to this page lives under the
 * `.evtfirst` wrapper so it can't collide with landing.css's `.hero`,
 * `.section-h2`, etc., and stays separate from `.evtpaid` / `.evtfree`.
 */
export function Events2026FirstPage() {
  ...
  // Event signup modal — this page is a single full package, so the
  // event code is always EVENT_04_PAID. `signupSource` traces the
  // origin button so we can attribute conversions later.
  const SIGNUP_CODE: EventCode = 'EVENT_04_PAID';
  ...
  useEffect(() => {
    const targets = Array.from(
      document.querySelectorAll<HTMLElement>(
        '.evtfirst .benefit-card, .evtfirst .event01-target, .evtfirst .compare-card, .evtfirst .launch-slot',
      ),
    );
  ...
  const canonicalUrl = `${origin}/events2026/first`;
  const docTitle =
    'AISEO 2026 런칭 이벤트 — 첫완성패키지(풀패키지)로 한 번에 제대로';
  const metaDescription =
    'AISEO 2026 EVENT 04 첫완성패키지. 001 검색노출전략 + 002 콘텐츠 기획·설계 + CORE 1 즉시 배포 전 과정에 사전상담 1시간까지 — 정가 50만원 → 35만원, 사전상담 1 + 본세션 3 총 4회로 디자인 방향까지 잡고 제대로 완성합니다.';
  ...
  return (
    <>
      ...
      {/*
        ...
        approach as `.evtfree`: every rule sits under `.evtfirst` so
      ...
      */}
      <style>{`
        ...
        .evtfirst .hero-stars {
          ...
          animation: evtfirstTwinkle 8s ease-in-out infinite;
        }
        @keyframes evtfirstTwinkle {
          0%, 100% { opacity: 0.7; }
          50% { opacity: 1; }
        }
        .evtfirst .hero-glow {
        ...
        .evtfirst .hero-badge {
          ...
          animation: evtfirstFadeUp .6s ease forwards .1s;
        }
        .evtfirst .hero-badge-dot {
          width: 6px; height: 6px;
          background: var(--accent); border-radius: 50%;
          animation: evtfirstPulse 2s infinite;
        }
        @keyframes evtfirstPulse {
        ...
        .evtfirst .hero-h1 .accent-text { color: var(--accent); }
        .evtfirst .hero-h1 .line {
          display: block;
          opacity: 0; transform: translateY(20px);
          animation: evtfirstFadeUp .7s ease forwards;
        }
        .evtfirst .hero-h1 .line:nth-child(1) { animation-delay: .2s; }
        .evtfirst .hero-h1 .line:nth-child(2) { animation-delay: .35s; }
        .evtfirst .hero-h1 .line:nth-child(3) { animation-delay: .5s; }
        .evtfirst .hero-sub {
          font-family: var(--font-ko);
          font-size: 16px; line-height: 1.85;
          color: rgba(255,255,255,0.7);
          max-width: 540px; margin: 0 auto;
          opacity: 0; animation: evtfirstFadeUp .6s ease forwards .7s;
        }
        ...
        .evtfirst .hero-prism {
          ...
          position: relative;
          opacity: 0; animation: evtfirstFadeUp .8s ease forwards .9s;
        }
        .evtfirst .hero-prism::before {
          content: '';
        ...
        .evtfirst .prism-svg {
          ...
          animation: evtfirstPrismFloat 8s ease-in-out infinite;
        }
        @keyframes evtfirstPrismFloat {
        ...
        .evtfirst .hero-video {
          ...
          opacity: 0; animation: evtfirstFadeUp .8s ease forwards 1.1s;
          z-index: 2;
        }
        .evtfirst .hero-video::before {
          content: '';
          position: absolute; inset: 0;
          background: radial-gradient(circle at center, rgba(196,168,245,0.1) 0%, transparent 60%);
          pointer-events: none;
        }
        .evtfirst .hero-video-play {
          position: absolute; left: 50%; top: 50%;
          transform: translate(-50%, -50%);
        ...
        .evtfirst .hero-video-play:hover {
          transform: translate(-50%, -50%) scale(1.08);
          box-shadow: 0 14px 40px rgba(196,168,245,0.4);
        }
        .evtfirst .hero-video-play svg { width: 22px; height: 22px; margin-left: 3px; }
        .evtfirst .hero-video-label {
        ...
        @keyframes evtfirstFadeUp { to { opacity: 1; transform: translateY(0); } }

        /* ── SECTION COMMON (override landing.css's section-* defaults) ── */
        .evtfirst section { padding: 120px 40px; }
        .evtfirst .section-inner { max-width: 1200px; margin: 0 auto; }
        .evtfirst .section-eyebrow {
          font-family: var(--font-en);
        ...
        .evtfirst .section-eyebrow::before,
        .evtfirst .section-eyebrow::after {
        ...
        .evtfirst .section-h2 .em { color: var(--accent-dark); }
        .evtfirst .section-sub {
          font-family: var(--font-ko);
        ...
        .evtfirst .benefits-header { margin-bottom: 56px; }
        .evtfirst .benefits-grid {
          display: grid;
        ...
        .evtfirst .benefit-card:hover {
          transform: translateY(-4px);
          box-shadow: var(--shadow-md);
          border-color: rgba(196,168,245,0.3);
        }
        .evtfirst .benefit-card.violet { background: linear-gradient(180deg, #F5EFFF 0%, #FAF6FF 100%); border-color: rgba(196,168,245,0.25); }
        .evtfirst .benefit-card.mint   { background: linear-gradient(180deg, #E8F7F0 0%, #F1FAF6 100%); border-color: rgba(127,200,166,0.25); }
        .evtfirst .benefit-card.cream  { background: linear-gradient(180deg, #FFF4E8 0%, #FFF9F1 100%); border-color: rgba(255,196,122,0.28); }
        .evtfirst .benefit-eyebrow {
          font-family: var(--font-en);
        ...
        .evtfirst .benefit-eyebrow .dot {
          display: inline-block; width: 4px; height: 4px;
          border-radius: 50%; background: var(--text-muted);
          margin: 0 8px; vertical-align: middle;
        }
        .evtfirst .benefit-title {
          font-family: var(--font-ko);
        ...
        .evtfirst .benefit-desc {
          font-size: 13.5px;
          color: var(--text-secondary);
          line-height: 1.7;
          flex: 1;
        }
        .evtfirst .benefit-foot {
          margin-top: 24px;
          padding-top: 16px;
        ...
        .evtfirst .pkg-section {
          background: var(--bg-soft);
          padding: 100px 40px;
          position: relative;
          overflow: hidden;
        }
        .evtfirst .pkg-section.alt { background: var(--bg); }
        .evtfirst .pkg-section::before {
          content: '';
          position: absolute;
          top: -160px; left: 50%;
          transform: translateX(-50%);
        ...
        /* tag pill 헤더 */
        .evtfirst .event01-tag-row { text-align: center; }
        .evtfirst .event01-tag {
        ...
        .evtfirst .pkg-section .section-h2 { margin-bottom: 16px; }
        .evtfirst .event01-lead {
          font-family: var(--font-ko);
          text-align: center;
          font-size: 16px;
          color: var(--text-secondary);
        ...
        .evtfirst .pkg-price-row .label {
          color: var(--text-primary);
          font-weight: 600;
          font-size: 16px;
          letter-spacing: -.2px;
        }
        .evtfirst .pkg-price-row .label small {
          display: block;
          font-size: 13px;
          color: var(--text-muted);
          margin-top: 4px;
        ...
        .evtfirst .pkg-price-divider {
          border-top: 1px dashed var(--border);
          margin: 14px 0 10px;
        }
        .evtfirst .pkg-price-sum {
          display: flex; align-items: center; justify-content: space-between;
          padding: 6px 0 18px;
          font-size: 15px;
          color: var(--text-muted);
        }
        .evtfirst .pkg-price-sum .num {
          font-family: var(--font-en);
          font-weight: 600;
          color: var(--text-secondary);
          font-size: 16px;
        }
        .evtfirst .pkg-price-final {
        ...
        .evtfirst .pkg-price-final .label {
          font-size: 14.5px;
          font-weight: 600;
          color: rgba(255,255,255,0.9);
          letter-spacing: .3px;
        }
        .evtfirst .pkg-price-final .num {
          font-family: var(--font-en);
          font-size: 38px;
          font-weight: 800;
        ...
        .evtfirst .pkg-price-final .num .unit {
          font-size: 18px;
          font-weight: 600;
          color: var(--accent);
          margin-left: 3px;
        }
        .evtfirst .pkg-price-saved {
          text-align: center;
          margin-top: 16px;
          font-size: 14px;
          color: var(--accent-deep);
        ...
        .evtfirst .core-notice-star::before {
          content: '★';
          color: #E8A60E;
          font-size: 14px;
          line-height: 1;
        }
        .evtfirst .core-notice-body { text-align: left; flex: 1; }
        .evtfirst .core-notice-title {
        ...
        .evtfirst .core-card:hover {
          border-color: rgba(196,168,245,0.4);
          box-shadow: var(--shadow-md);
        }
        .evtfirst .core-card-header {
          display: grid;
        ...
        .evtfirst .core-card-body { min-width: 0; }
        .evtfirst .core-card-label {
          display: inline-flex;
          align-items: center;
        ...
        .evtfirst .core-card-tags {
          display: flex; flex-wrap: wrap; gap: 6px;
        }
        .evtfirst .core-card-tag {
          font-size: 12.5px;
          color: var(--text-muted);
          background: var(--bg-soft);
        ...
        .evtfirst .core-card-price {
          text-align: right;
          flex-shrink: 0;
          display: flex; align-items: center; gap: 6px;
        }
        .evtfirst .core-card-price-strike {
          font-size: 13px;
          color: var(--text-muted);
        ...
        .evtfirst .core-card-price-arrow {
          color: var(--accent-dark);
          font-weight: 700;
          margin: 0 2px;
        }
        .evtfirst .core-card-price-free {
          font-size: 17px;
          font-weight: 800;
          color: var(--accent-deep);
          letter-spacing: -.4px;
        }
        .evtfirst .core-card-toggle {
        ...
        .evtfirst .core-card-toggle:hover {
          background: var(--accent-light);
          border-color: rgba(196,168,245,0.4);
        }
        .evtfirst .core-card-toggle svg {
          width: 16px; height: 16px;
          transition: transform .35s cubic-bezier(0.65, 0.05, 0.35, 1);
        }
        .evtfirst .core-card-toggle[aria-expanded="true"] svg {
          transform: rotate(180deg);
        }
        .evtfirst .core-card-expand {
          overflow: hidden;
          max-height: 0;
          transition: max-height .5s cubic-bezier(0.65, 0.05, 0.35, 1);
        }
        .evtfirst .core-card-expand.open { max-height: 1500px; }
        .evtfirst .core-card-expand-inner {
        ...
        .evtfirst .core-col-section + .core-col-section { margin-top: 26px; }
        .evtfirst .core-col-h {
        ...
        .evtfirst .core-col-section.muted .core-col-h {
          background: var(--text-primary);
        }
        .evtfirst .core-col-desc {
          font-size: 14.5px;
          color: var(--text-secondary);
          line-height: 1.75;
        }
        .evtfirst .core-col-desc + .core-col-desc { margin-top: 14px; }
        .evtfirst .core-col-list {
          list-style: none;
          display: flex; flex-direction: column;
          gap: 9px;
          padding: 0; margin: 0;
        }
        .evtfirst .core-col-list.bullet li {
          font-size: 14px;
          color: var(--text-secondary);
          line-height: 1.6;
          padding-left: 16px;
        ...
        .evtfirst .core-col-list.bullet li::before {
          content: '';
          position: absolute;
          left: 0; top: 9px;
        ...
        .evtfirst .core-col-list.check li {
          font-size: 14px;
          color: var(--text-secondary);
          line-height: 1.6;
          padding-left: 22px;
        ...
        .evtfirst .core-col-list.check li::before {
          content: '✓';
          position: absolute;
          left: 0; top: 0;
          color: var(--accent-dark);
        ...
        .evtfirst .core-col-chips {
          display: flex; flex-wrap: wrap; gap: 8px;
          margin-top: 4px;
        }
        .evtfirst .core-col-chip {
        ...
        .evtfirst .core-col-chip-icon { font-size: 14px; line-height: 1; }

        /* PKG SECTION CTA */
        .evtfirst .event01-cta {
          margin-top: 64px;
          text-align: center;
        }
        .evtfirst .event01-cta-row {
          display: inline-flex;
          gap: 12px;
          flex-wrap: wrap;
          justify-content: center;
        }
        .evtfirst .event01-btn {
          display: inline-flex; align-items: center; gap: 10px;
        ...
        .evtfirst .event01-btn:hover {
          background: var(--accent-deep);
          transform: translateY(-2px);
          box-shadow: 0 10px 28px rgba(139,111,212,0.35);
        }
        .evtfirst .event01-btn.secondary {
          background: var(--bg-card);
          color: var(--text-primary);
          border-color: var(--border);
          box-shadow: none;
        }
        .evtfirst .event01-btn.secondary:hover {
          border-color: var(--accent);
          background: var(--bg-soft);
          box-shadow: var(--shadow-sm);
        }
        .evtfirst .event01-btn-arrow { transition: transform .2s; display: inline-block; }
        .evtfirst .event01-btn:hover .event01-btn-arrow { transform: translateX(3px); }
        .evtfirst .event01-cta-note {
          margin-top: 18px;
          font-size: 12.5px;
          color: var(--text-muted);
          letter-spacing: .2px;
        ...
        /* ── 공통 타겟 ── */
        .evtfirst .common-targets {
          background: var(--bg);
          padding: 100px 40px 80px;
        }
        .evtfirst .common-targets-inner {
          max-width: 1100px;
          margin: 0 auto;
        }
        .evtfirst .event01-targets {
          display: grid;
        ...
        .evtfirst .event01-target:hover {
          border-color: rgba(196,168,245,0.35);
          transform: translateY(-2px);
        }
        .evtfirst .event01-target-icon {
          width: 28px; height: 28px;
          border-radius: 50%;
          background: var(--accent-light);
          display: flex; align-items: center; justify-content: center;
        ...
        .evtfirst .event01-target-icon svg {
          width: 14px; height: 14px;
          stroke: var(--accent-dark);
          stroke-width: 2.5;
          fill: none;
        }
        .evtfirst .event01-target-text {
          font-family: var(--font-ko);
          font-size: 16.5px;
          color: var(--text-primary);
          line-height: 1.6;
        ...
        .evtfirst .faq-list { margin-top: 48px; }
        .evtfirst .faq-item {
          background: var(--bg-card);
          border: 1px solid var(--border);
          border-radius: var(--radius-md);
        ...
        .evtfirst .faq-q-icon svg { width: 15px; height: 15px; }
        .evtfirst .faq-q[aria-expanded="true"] .faq-q-icon { transform: rotate(180deg); }
        .evtfirst .faq-a {
          overflow: hidden;
          max-height: 0;
          transition: max-height .4s cubic-bezier(0.65, 0.05, 0.35, 1);
        }
        .evtfirst .faq-a.open { max-height: 500px; }
        .evtfirst .faq-a-inner {
        ...
        .evtfirst .faq-a-inner strong {
          color: var(--text-primary);
          font-weight: 700;
        }

        /* ── A vs B 비교 ── */
        .evtfirst .compare-section {
          background: var(--bg);
          padding: 100px 40px 120px;
        }
        .evtfirst .compare-inner {
          max-width: 1100px;
          margin: 0 auto;
        }
        .evtfirst .compare-grid {
          display: grid;
          grid-template-columns: 1fr 1fr;
          gap: 22px;
          margin-top: 56px;
        ...
        .evtfirst .compare-card:hover {
          transform: translateY(-4px);
          box-shadow: var(--shadow-md);
          border-color: rgba(196,168,245,0.4);
        }
        .evtfirst .compare-card-icon {
          font-size: 40px;
          margin-bottom: 18px;
          line-height: 1;
        }
        .evtfirst .compare-card-pkg {
          font-family: var(--font-en);
          font-size: 12px;
          font-weight: 800;
          color: var(--accent-deep);
        ...
        .evtfirst .compare-card-list li::before {
          content: '✓';
          position: absolute;
          left: 0; top: 0;
          color: var(--accent-dark);
        ...
        .evtfirst .launch-cta-inner {
          position: relative; z-index: 1;
          max-width: 980px;
          margin: 0 auto;
          text-align: center;
        }
        .evtfirst .launch-cta-eyebrow {
          display: inline-flex; align-items: center; gap: 10px;
        ...
        .evtfirst .launch-cta-eyebrow .star {
          color: #FFD75A;
          font-size: 13px;
          line-height: 1;
        }
        .evtfirst .launch-cta-eyebrow .pulse {
          width: 7px; height: 7px;
          border-radius: 50%;
          background: #4ADE80;
          animation: evtfirstLaunchPulse 2s infinite;
        }
        @keyframes evtfirstLaunchPulse {
          0%, 100% { opacity: 1; box-shadow: 0 0 0 0 rgba(74, 222, 128, 0.6); }
          50% { opacity: 0.6; box-shadow: 0 0 0 8px rgba(74, 222, 128, 0); }
        }
        .evtfirst .launch-cta-h {
          font-family: var(--font-ko);
        ...
        .evtfirst .launch-cta-h .accent { color: var(--accent); }
        .evtfirst .launch-cta-sub {
          font-size: 16px;
          color: rgba(255,255,255,0.72);
          line-height: 1.7;
          margin-bottom: 48px;
        }
        .evtfirst .launch-cta-sub strong {
          color: #fff;
          font-weight: 600;
        }

        /* 카운트다운 */
        .evtfirst .launch-countdown {
          display: inline-flex;
          gap: 12px;
          margin-bottom: 48px;
          padding: 20px 28px;
        ...
        .evtfirst .launch-countdown-label {
          align-self: center;
          font-family: var(--font-en);
          font-size: 11px;
          font-weight: 700;
        ...
        .evtfirst .launch-countdown-unit {
          text-align: center;
          min-width: 60px;
        }
        .evtfirst .launch-countdown-num {
          font-family: var(--font-en);
          font-size: 32px;
          font-weight: 800;
          color: #fff;
        ...
        .evtfirst .launch-countdown-name {
          font-family: var(--font-en);
          font-size: 10px;
          font-weight: 600;
        ...
        .evtfirst .launch-live.show { display: inline-flex; }
        .evtfirst .launch-live::before {
          content: '';
        ...
        .evtfirst .launch-slot:hover {
          background: rgba(196,168,245,0.12);
          border-color: rgba(196,168,245,0.5);
          transform: translateY(-3px);
        }
        .evtfirst .launch-slot-head {
          display: flex; align-items: center; justify-content: space-between;
          margin-bottom: 14px;
        }
        .evtfirst .launch-slot-pkg {
          font-family: var(--font-en);
          font-size: 11px;
          font-weight: 800;
          color: var(--accent);
        ...
        .evtfirst .launch-slot:hover .launch-slot-cta { color: #fff; }
        .evtfirst .launch-slot-cta-arrow { transition: transform .25s; display: inline-block; }
        .evtfirst .launch-slot:hover .launch-slot-cta-arrow {
          transform: translateX(3px);
        }
        .evtfirst .launch-cta-note {
          font-size: 13px;
          color: rgba(255,255,255,0.55);
          line-height: 1.65;
          max-width: 520px;
        ...
        .evtfirst .cross-link-inner {
          position: relative; z-index: 1;
          max-width: 1100px;
          margin: 0 auto;
          text-align: center;
        }
        .evtfirst .cross-link-eyebrow {
          display: inline-flex; align-items: center; gap: 10px;
          font-family: var(--font-en);
          font-size: 12px;
          font-weight: 700;
        ...
        .evtfirst .cross-link-eyebrow::before,
        .evtfirst .cross-link-eyebrow::after {
          content: ''; width: 24px; height: 1px;
          background: rgba(255,255,255,0.3);
        }
        .evtfirst .cross-link-h {
          font-family: var(--font-ko);
        ...
        .evtfirst .cross-link-h .em { color: var(--accent); }
        .evtfirst .cross-link-sub {
          font-size: 16px;
          color: rgba(255,255,255,0.7);
          line-height: 1.75;
          max-width: 560px;
        ...
        .evtfirst .cross-card:hover {
          background: rgba(255,255,255,0.08);
          border-color: rgba(196,168,245,0.45);
          transform: translateY(-3px);
        }
        .evtfirst .cross-card-badge {
          width: 64px; height: 64px;
          border-radius: 16px;
          background: linear-gradient(135deg, var(--accent) 0%, var(--accent-deep) 100%);
          display: flex; flex-direction: column;
        ...
        .evtfirst .cross-card-body { min-width: 0; }
        .evtfirst .cross-card-pkg {
          font-family: var(--font-en);
          font-size: 11px;
          font-weight: 800;
          color: var(--accent);
        ...
        .evtfirst .cross-card-desc {
          font-size: 14.5px;
          color: rgba(255,255,255,0.62);
          line-height: 1.55;
        }
        .evtfirst .cross-card-arrow {
        ...
        .evtfirst .cross-card:hover .cross-card-arrow {
          background: var(--accent);
          border-color: var(--accent);
          transform: translateX(3px);
        }
        .evtfirst .cross-card-arrow svg { width: 18px; height: 18px; }

        @media (max-width: 900px) {
          .evtfirst section { padding: 80px 24px; }
          .evtfirst .hero { padding: 120px 24px 60px; min-height: auto; }
          .evtfirst .hero-prism { width: 160px; height: 160px; }
          .evtfirst .benefits-grid { grid-template-columns: 1fr; }
          .evtfirst .pkg-section { padding: 60px 24px; }
          .evtfirst .pkg-price { padding: 26px 24px; }
          .evtfirst .pkg-price-final { padding: 18px 20px; }
          .evtfirst .pkg-price-final .num { font-size: 30px; }
          .evtfirst .pkg-price-row .label { font-size: 14.5px; }
          .evtfirst .pkg-price-row .label small { font-size: 12px; }
          .evtfirst .pkg-price-row .num { font-size: 15px; }
          .evtfirst .core-card-header { grid-template-columns: auto 1fr auto; gap: 14px; padding: 18px 20px; }
          .evtfirst .core-card-toggle { grid-column: 3; }
          .evtfirst .core-card-price { grid-column: 1 / -1; justify-content: flex-end; padding-top: 10px; border-top: 1px dashed var(--border); }
          .evtfirst .core-card-expand-inner { grid-template-columns: 1fr; gap: 26px; padding: 24px 22px; }
          .evtfirst .core-notice { padding: 16px 20px; }
          .evtfirst .core-notice-title { font-size: 14px; }
          .evtfirst .core-notice-sub { font-size: 13px; }
          .evtfirst .common-targets { padding: 80px 24px 60px; }
          .evtfirst .event01-targets { grid-template-columns: 1fr; }
          .evtfirst .faq-section { padding: 60px 24px; }
          .evtfirst .faq-q { padding: 20px 24px; }
          .evtfirst .faq-q-text { font-size: 16px; }
          .evtfirst .faq-a-inner { padding: 16px 24px 24px; font-size: 14.5px; }
          .evtfirst .compare-section { padding: 60px 24px 80px; }
          .evtfirst .compare-grid { grid-template-columns: 1fr; gap: 30px; }
          .evtfirst .compare-grid::before { display: none; }
          .evtfirst .launch-cta { padding: 60px 24px 80px; }
          .evtfirst .launch-countdown { gap: 8px; padding: 16px 18px; flex-wrap: wrap; justify-content: center; }
          .evtfirst .launch-countdown-label { padding-right: 0; margin-right: 0; border-right: none; padding-bottom: 8px; flex-basis: 100%; text-align: center; }
          .evtfirst .launch-countdown-unit { min-width: 52px; }
          .evtfirst .launch-countdown-num { font-size: 26px; }
          .evtfirst .launch-slots { grid-template-columns: 1fr; gap: 12px; }
          .evtfirst .launch-slot { padding: 20px 18px; }
          .evtfirst .cross-link { padding: 60px 24px 80px; }
          .evtfirst .cross-card { grid-template-columns: auto 1fr; gap: 16px; padding: 22px 22px; }
          .evtfirst .cross-card-arrow { grid-column: 1 / -1; justify-self: flex-end; width: 40px; height: 40px; }
          .evtfirst .cross-card-h { font-size: 18px; }
          .evtfirst .cross-card-badge { width: 56px; height: 56px; }
        }
        @media (max-width: 480px) {
          .evtfirst .hero-h1 { letter-spacing: -1.5px; }
          .evtfirst .section-h2 { letter-spacing: -.8px; }
          .evtfirst .event01-cta-row { flex-direction: column; width: 100%; }
          .evtfirst .event01-btn { width: 100%; justify-content: center; }
        }
      `}</style>
          ...
          <div className="nav-links">
            ...
            <div className="nav-item-has-sub">
              ...
              <div className="nav-submenu" role="menu">
                ...
                <a href="/events2026/first" className="active" role="menuitem">첫완성패키지</a>
              </div>
            </div>
            <a href="/blog">블로그</a>
          </div>
      ...
      <div className="evtfirst">
        ...
        <section className="hero">
          ...
          <div className="hero-content">
            ...
            <h1 className="hero-h1">
              <span className="line">검색·기획·배포,</span>
              <span className="line"><span className="accent-text">전 과정을</span> 한 번에</span>
              <span className="line">제대로 완성합니다</span>
            </h1>
            <p className="hero-sub">
              001·002·CORE1 세 과정을 모두 포함하는 첫완성패키지.<br />
              사전상담으로 디자인 방향을 먼저 잡고, 검색 노출까지 빠짐없이 끝냅니다.
            </p>

            <div className="hero-prism" aria-hidden="true">
              <svg className="prism-svg" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
                <defs>
                  <linearGradient id="evtfirstPrismGrad1" x1="0%" y1="0%" x2="100%" y2="100%">
                    <stop offset="0%" stopColor="#C4A8F5" />
                    <stop offset="50%" stopColor="#9BB8F8" />
                    <stop offset="100%" stopColor="#F5C4E8" />
                  </linearGradient>
                  <linearGradient id="evtfirstPrismGrad2" x1="0%" y1="0%" x2="100%" y2="100%">
                    <stop offset="0%" stopColor="#FFD4A8" />
                    <stop offset="100%" stopColor="#C4A8F5" />
                  </linearGradient>
                  <linearGradient id="evtfirstPrismGrad3" x1="0%" y1="0%" x2="0%" y2="100%">
                    <stop offset="0%" stopColor="#9BB8F8" stopOpacity="0.9" />
                    <stop offset="100%" stopColor="#6B4FB8" stopOpacity="0.6" />
                  </linearGradient>
                </defs>
                <polygon points="100,30 160,90 130,160 70,160 40,90" fill="url(#evtfirstPrismGrad1)" opacity="0.85" />
                <polygon points="100,30 160,90 100,100" fill="url(#evtfirstPrismGrad2)" opacity="0.7" />
                <polygon points="100,100 160,90 130,160" fill="url(#evtfirstPrismGrad3)" opacity="0.8" />
                <polygon points="100,100 130,160 70,160" fill="url(#evtfirstPrismGrad1)" opacity="0.6" />
                <polygon points="100,100 70,160 40,90" fill="url(#evtfirstPrismGrad2)" opacity="0.5" />
                <polygon points="100,30 100,100 40,90" fill="url(#evtfirstPrismGrad3)" opacity="0.7" />
        ...
        <section className="benefits">
          <div className="section-inner">
            <div className="benefits-header">
              <div className="section-eyebrow">전 과정 한 번에</div>
              <h2 className="section-h2">
                세 과정을 모두 담은<br /><span className="em">첫완성패키지</span>
              </h2>
              <p className="section-sub" style={{ marginTop: 14 }}>
                둘 중 하나를 고르는 대신, 검색 전략·콘텐츠 기획·배포를 전부 포함합니다.<br />
                사전상담으로 디자인 방향을 먼저 잡고 처음부터 끝까지 제대로 완성합니다.
              </p>
            </div>

            <div className="benefits-grid">
              <a href="#event04" className="benefit-card violet">
                <div className="benefit-eyebrow">001 <span className="dot"></span> 사전준비</div>
                <h3 className="benefit-title">🎯 검색 전략</h3>
                <p className="benefit-desc">업종 분석 · 키워드 전략 · 경쟁군 분석으로 검색 노출 구조를 먼저 설계합니다.</p>
                <div className="benefit-foot">001 SEO 검색노출전략 점검 · 정가 20만원</div>
              </a>

              <a href="#event04" className="benefit-card mint">
                <div className="benefit-eyebrow">002 <span className="dot"></span> 사전준비</div>
                <h3 className="benefit-title">✍️ 콘텐츠 기획</h3>
                <p className="benefit-desc">메뉴 구조 · 서비스 설명 · CTA · 톤앤매너까지 홈페이지에 담을 모든 것을 기획합니다.</p>
                <div className="benefit-foot">002 홈페이지·콘텐츠 기획·설계 · 정가 20만원</div>
              </a>

              <a href="#event04" className="benefit-card cream">
                <div className="benefit-eyebrow">CORE1 <span className="dot"></span> 핵심 교육</div>
                <h3 className="benefit-title">🚀 즉시 배포</h3>
                <p className="benefit-desc">실제 웹 주소에 올리고 Search Console · GA4 · 기술적 SEO 연동까지 한 번에.</p>
        ...
        {/* ══════════════════════════════════════════════
              EVENT 04 — 풀패키지 (001 + 002 + CORE1 + 사전상담)
        ══════════════════════════════════════════════ */}
        <section className="pkg-section" id="event04">
          <div className="pkg-section-inner">
            <div className="event01-tag-row">
              <div className="event01-tag">
                <span className="num">4</span>
                <span>EVENT 04 · 풀패키지</span>
              </div>
            </div>

            <h2 className="section-h2">
              검색·기획·배포,<br />
              <span className="em">전 과정을 한 번에</span> 제대로
            </h2>
            <p className="event01-lead">
              001·002·CORE1 세 과정을 모두 포함합니다.<br />
              사전상담으로 원하는 디자인 방향을 먼저 잡고, 검색 노출까지 빠짐없이 완성합니다.
            </p>
            ...
            <div className="pkg-price">
              ...
              <div className="pkg-price-row">
                <span className="label">002 홈페이지 및 콘텐츠 기획 · 설계<small>메뉴 구조 · CTA 작성 · 콘텐츠 기획</small></span>
                <span className="num">20만원</span>
              </div>
              <div className="pkg-price-row">
              ...
              <div className="pkg-price-final">
                <span className="label">EVENT 04 풀패키지 가격</span>
                <span className="num">35<span className="unit">만원</span></span>
              </div>
              <div className="pkg-price-saved">15만원 절감 · 30% off · 사전상담 1시간 추가 포함</div>
            </div>
            ...
            <div className="free-bundle-core">
              ...
              <div className="core-card">
                <div className="core-card-header">
                  ...
                  <button
                    ...
                    aria-controls="evtfirst-expand-001"
                ...
                <div className={`core-card-expand${isOpen('001') ? ' open' : ''}`} id="evtfirst-expand-001">
              ...
              <div className="core-card">
                <div className="core-card-header">
                  ...
                  <button
                    ...
                    aria-controls="evtfirst-expand-002"
                ...
                <div className={`core-card-expand${isOpen('002') ? ' open' : ''}`} id="evtfirst-expand-002">
                  <div className="core-card-expand-inner">
                    <div>
                      <div className="core-col-section">
                        <p className="core-col-desc">
              ...
              <div className="core-card">
                <div className="core-card-header">
                  ...
                  <button
                    ...
                    aria-controls="evtfirst-expand-core-02"
                ...
                <div className={`core-card-expand${isOpen('core-02') ? ' open' : ''}`} id="evtfirst-expand-core-02">
                  <div className="core-card-expand-inner">
                    <div>
                      <div className="core-col-section">
                        <p className="core-col-desc">
              ...
              <div className="core-notice">
                <div className="core-notice-star" aria-hidden="true"></div>
                <div className="core-notice-body">
                  <div className="core-notice-title">PLUS — 사전상담 1시간</div>
                  <div className="core-notice-sub">풀패키지에만 더해지는 추가 혜택입니다</div>
                </div>
              </div>
              ...
              <div className="core-card">
                <div className="core-card-header">
                  <div className="core-card-icon">
                    <span className="core-card-icon-text">PLUS</span>
                  </div>
                  <div className="core-card-body">
                    <span className="core-card-label">PLUS · Pre-consult</span>
                    <h4 className="core-card-h">사전상담 1시간 — 디자인 방향 먼저 확정</h4>
                    <div className="core-card-tags">
                      <span className="core-card-tag">#사전상담</span>
                      <span className="core-card-tag">#디자인방향</span>
                      <span className="core-card-tag">#풀패키지전용</span>
                    </div>
                  </div>
                  <div className="core-card-price">
                    <span className="core-card-price-free">추가 포함</span>
                  </div>
                  <button
                    type="button"
                    className="core-card-toggle"
                    aria-expanded={isOpen('precall')}
                    aria-controls="evtfirst-expand-precall"
                    aria-label={isOpen('precall') ? '접기' : '자세히 보기'}
                    onClick={() => toggleCard('precall')}
                  >
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                      <polyline points="6 9 12 15 18 9" />
                    </svg>
                ...
                <div className={`core-card-expand${isOpen('precall') ? ' open' : ''}`} id="evtfirst-expand-precall">
                  <div className="core-card-expand-inner">
                    <div>
                      <div className="core-col-section">
                        <p className="core-col-desc">
                          본세션을 시작하기 전에, 원하는 디자인 방향과 사이트의 분위기를 먼저 맞추는 시간입니다. 레퍼런스·톤·색감·구성 방향을 미리 합의해 두면, 이후 001·002·CORE1 전 과정이 사장님이 원하는 결과로 곧장 이어집니다.
                        </p>
                        <p className="core-col-desc">
                          "둘 중 하나 고르기"가 아니라 "전부 + 사전상담으로 제대로 완성" — 이것이 풀패키지의 차이입니다.
                        </p>
                      </div>
                      <div className="core-col-section">
                        <span className="core-col-h">이런 분께 추천합니다</span>
                        <ul className="core-col-list bullet">
                          <li>검색·기획·배포를 한 번에 빠짐없이 끝내고 싶은 분</li>
                          <li>디자인 방향부터 제대로 잡고 시작하고 싶은 분</li>
                          <li>각 과정을 따로 고민하기보다 전 과정을 맡기고 싶은 분</li>
                        </ul>
                      </div>
                    </div>
                    <div>
                      <div className="core-col-section">
                        <span className="core-col-h">진행 구성</span>
                        <ul className="core-col-list check">
                          <li>사전상담 1회 (디자인 방향 확정)</li>
                          <li>본세션 3회 (001 · 002 · CORE1)</li>
                          <li>총 4회 · 회당 1시간</li>
                        </ul>
                      </div>
                      <div className="core-col-section muted">
                        <span className="core-col-h">진행 방식</span>
                        <div className="core-col-chips">
                          <span className="core-col-chip"><span className="core-col-chip-icon">⏱</span>총 4회 · 회당 60분</span>
                          <span className="core-col-chip"><span className="core-col-chip-icon">💬</span>화상 진행</span>
                          <span className="core-col-chip"><span className="core-col-chip-icon">🎨</span>디자인 방향서</span>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="event01-cta">
              <div className="event01-cta-row">
                <button
                  ...
                  onClick={() => openSignup('first-event04-cta')}
                >
                  <span>EVENT 04 풀패키지 신청하기</span>
                  <span className="event01-btn-arrow">→</span>
                </button>
                <a href="/events2026/paid" className="event01-btn secondary">
                  <span>EVENT 02·03 비교해보기</span>
                </a>
              </div>
              <p className="event01-cta-note">정가 50만원 → 35만원 · 사전상담 1 + 본세션 3 = 총 4회</p>
            </div>
          </div>
        </section>
          ...
          <div className="faq-inner">
            ...
            <div className="faq-list">
              {[
                {
                  id: 'faq-1',
                  q: 'EVENT 04 풀패키지는 무엇을 포함하나요?',
                  a: (
                    <>
                      <strong>001 검색노출전략 점검 + 002 콘텐츠 기획·설계 + CORE 1 즉시 배포</strong> 세 과정을 모두 포함합니다. 여기에 <strong>사전상담 1시간</strong>이 더해져, 디자인 방향을 먼저 확정한 뒤 검색·기획·배포 전 과정을 빠짐없이 완성합니다. (사전상담 1 + 본세션 3 = 총 4회)
                    </>
                  ),
                },
                {
                  id: 'faq-2',
                  q: 'EVENT 02·03과 무엇이 다른가요?',
                  a: (
                    <>
                      EVENT 02·03이 <strong>"둘 중 하나 고르기"</strong>(검색 전략 또는 콘텐츠 기획)라면, EVENT 04는 <strong>"전부 + 사전상담으로 제대로 완성"</strong>입니다. 001·002·CORE1 전 과정에 사전상담까지 더해 정가 50만원 → 35만원으로 진행합니다.
                    </>
                  ),
                },
                {
                  ...
                  a: (
                    <>
                      EVENT 01은 선착순 3분 한정 무료 프로그램으로 자격 검토를 거쳐 진행됩니다. EVENT 04는 자격 검토 없이 누구나 신청 가능한 정식 풀패키지로, 사전상담과 001·002·CORE1 전 과정이 모두 포함되어 가장 깊이 있게 진행됩니다.
                    </>
                  ),
                },
                {
        ...
        <section className="compare-section">
          <div className="compare-inner">
            <div className="section-eyebrow">WHY FULL PACKAGE</div>
            <h2 className="section-h2">
              둘 중 하나가 아니라<br /><span className="em">전부, 제대로</span>
            </h2>
            <p className="event01-lead">
              EVENT 02·03이 출발점 하나를 고르는 패키지라면,<br />
              EVENT 04는 검색·기획·배포 전부에 사전상담까지 더한 풀패키지입니다.
            </p>

            <div className="compare-grid">
              <article className="compare-card">
                <div className="compare-card-icon" aria-hidden="true">🔀</div>
                <div className="compare-card-pkg">EVENT 02 · 03 · 둘 중 하나</div>
                <h3 className="compare-card-h">출발점 하나를<br />골라서 시작</h3>
                <p className="compare-card-sub">
                  검색 전략(001) 또는 콘텐츠 기획(002) 중 하나에 CORE 1 배포를 더한 2단계 패키지. 사장님 상황에 맞는 한 가지를 골라 합리적으로 시작합니다.
                </p>
                <ul className="compare-card-list">
                  <li>001 또는 002 중 하나 + CORE 1</li>
                  <li>각 정가 30만원 → 10만원</li>
                  <li>출발점이 분명한 분께 적합</li>
                  <li>사전상담은 포함되지 않음</li>
                </ul>
                <a href="/events2026/paid" className="compare-cta">
                  <span>EVENT 02·03 보러가기</span>
              ...
              <article className="compare-card">
                <div className="compare-card-icon" aria-hidden="true">✅</div>
                <div className="compare-card-pkg">EVENT 04 · 풀패키지</div>
                <h3 className="compare-card-h">전부 + 사전상담으로<br />제대로 완성</h3>
                <p className="compare-card-sub">
                  001·002·CORE1 세 과정을 모두 포함하고, 사전상담 1시간으로 디자인 방향까지 먼저 확정합니다. 처음부터 끝까지 빠짐없이 완성합니다.
                </p>
                <ul className="compare-card-list">
                  <li>001 + 002 + CORE 1 전 과정</li>
                  <li>정가 50만원 → 35만원 (30% off)</li>
                  <li>사전상담 1 + 본세션 3 = 총 4회</li>
                  <li>디자인 방향 확정 후 제대로 완성</li>
                </ul>
                <a href="#event04" className="compare-cta">
                  <span>EVENT 04 자세히 보기</span>
                  <span aria-hidden="true">↑</span>
                </a>
              </article>
            </div>
        ...
        <section className="launch-cta">
          <div className="launch-cta-inner">
            ...
            <h2 className="launch-cta-h">
              풀패키지 <span className="accent">선착순 5팀</span>만<br />
              신청 가능합니다
            </h2>
            ...
            <div className="launch-slots">
              <button
                ...
                onClick={() => openSignup('first-launch-slot')}
              >
                <div className="launch-slot-head">
                  <span className="launch-slot-pkg">EVENT 04 · 풀패키지</span>
                  <span className="launch-slot-count">5팀 한정</span>
                </div>
                <h4 className="launch-slot-h">🚀 검색·기획·배포 전 과정 + 사전상담</h4>
            ...
            <p className="launch-cta-note">
              EVENT 04 풀패키지 선착순 5팀 한정 · 신청 후 카카오톡으로 일정 협의<br />
