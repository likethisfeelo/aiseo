...
const statusDot = (active: boolean) => ({
  ...
  background: active ? 'var(--success)' : 'var(--border-strong)', marginRight: 6,
} as const);
...
function StatusBanner({ active, activeText, inactiveText }: { active: boolean; activeText: string; inactiveText: string }) {
  return (
    <div style={{
      ...
      background: active ? 'var(--success-soft)' : 'var(--danger-soft)',
      ...
      <span style={{ color: active ? 'var(--success-dark)' : 'var(--danger-dark)', fontWeight: 600 }}>
...
function GtmTab({ snippets, onChange }: { snippets: HeadSnippets; onChange: (s: HeadSnippets) => void }) {
  return (
    <div>
      ...
      <div style={cardStyle}>
        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 12 }}>GTM에서 관리할 수 있는 태그</div>
        {['GA4 (Google Analytics 4)', 'Meta Pixel (Facebook/Instagram)', 'Kakao Pixel', 'Google Ads 전환 추적', 'Hotjar / Microsoft Clarity'].map((tag) => (
          <div key={tag} style={{ padding: '6px 0', fontSize: 12, color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ color: 'var(--text-muted)' }}>&#9634;</span> {tag}
          </div>
        ))}
      </div>
    ...
    <div>
      ...
      {!snippets.ga4Id && (
        <div style={{ ...cardStyle, background: 'var(--warning-soft)', borderColor: 'var(--warning-soft)' }}>
          <div style={{ fontSize: 12, color: 'var(--warning-dark)' }}>
      ...
      <div style={cardStyle}>
        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 12 }}>핵심 GA4 리포트</div>
        {[
          ...
          <div key={item.title} style={{ padding: '10px 0', borderBottom: '1px solid var(--border-soft)' }}>
      ...
      <div style={cardStyle}>
        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 12 }}>추천 이벤트</div>
...
function MetaPixelTab({ snippets, onChange }: { snippets: HeadSnippets; onChange: (s: HeadSnippets) => void }) {
  return (
    <div>
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {['PageView', 'ViewContent', 'AddToCart', 'Purchase', 'Lead', 'CompleteRegistration', 'Search', 'Contact'].map((ev) => (
            <span key={ev} style={{
              padding: '4px 10px', borderRadius: 12, fontSize: 11, fontWeight: 500,
              background: 'var(--primary-soft)', color: 'var(--primary)', border: '1px solid var(--primary-soft-strong)',
            }}>{ev}</span>
          ))}
        </div>
      </div>
      ...
      <div style={cardStyle}>
        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 8 }}>Meta Pixel Helper</div>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 16, flexWrap: 'wrap' }}>
        ...
        <ExternalLink href="https://developers.facebook.com/tools/debug/" label="공유 디버거" bg="var(--text-secondary)" />
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div style={cardStyle}>
        ...
        <input
          ...
          style={inputStyle}
        />
        <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>카카오톡 채널 홈 URL에서 확인할 수 있습니다</div>
      </div>

      <div style={cardStyle}>
        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 8 }}>공유 링크 디버거</div>
        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 12 }}>
          카카오톡으로 공유할 때 OG 태그가 제대로 표시되는지 확인하고, 캐시를 초기화할 수 있습니다.
        </div>
        {snippets.ogTitle && (
          <div style={{ padding: 10, borderRadius: 6, background: 'var(--bg-soft)', border: '1px solid var(--border)', marginBottom: 12 }}>
            <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4 }}>현재 OG 태그 설정:</div>
            <div style={{ fontSize: 12, color: 'var(--text-primary)' }}><strong>제목:</strong> {snippets.ogTitle}</div>
            {snippets.ogDescription && <div style={{ fontSize: 12, color: 'var(--text-primary)' }}><strong>설명:</strong> {snippets.ogDescription}</div>}
            {snippets.ogImage && <div style={{ fontSize: 12, color: 'var(--text-primary)' }}><strong>이미지:</strong> 설정됨</div>}
          </div>
        )}
        <ExternalLink href="https://developers.kakao.com/tool/debugger/sharing" label="카카오 공유 디버거" bg="#FEE500" color="var(--text-primary)" />
      </div>

      <div style={{ display: 'flex', gap: 8, marginTop: 16, flexWrap: 'wrap' }}>
        <ExternalLink href="https://business.kakao.com" label="카카오 비즈보드" bg="#FEE500" color="var(--text-primary)" />
      </div>
    </div>
  );
}
    ...
    <div style={{ maxWidth: 720, margin: '0 auto', padding: 20 }}>
      ...
      {tab !== 'ga4' && (
        <div style={{ marginTop: 24, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {msg && <span style={{ fontSize: 12, color: msg.includes('실패') ? 'var(--danger)' : 'var(--success)' }}>{msg}</span>}
        </div>
      )}
    </div>
  );
