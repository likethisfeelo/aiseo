...
const handleSubmit = async (event) => {
  ...
  if (!name || !phone || !kakaoConsent) {
    return badRequest('이름, 연락처, 카카오톡 연락 동의는 필수입니다.', event);
  }
  ...
  return ok({ consultationId }, event);
};
...
exports.handler = async (event) => {
  try {
    ...
    if (!table) return serverError('CONSULTATIONS_TABLE is not configured', event);
    ...
    return serverError('Failed to process consultation request', event);
  }
};
