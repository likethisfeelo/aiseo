...
const handleGet = async (event, sitesTable) => {
  ...
  return ok({ services: result.Item.services || [] });
};
...
const handlePost = async (event, sitesTable) => {
  ...
  if (!service || !service.name) return badRequest('Service name is required');
  ...
  return ok({ service: sanitized, services });
};
...
const handleDelete = async (event, sitesTable) => {
  ...
  if (!siteId || !serviceId) return badRequest('siteId and serviceId are required');
...
exports.handler = async (event) => {
  try {
    ...
    return serverError('Failed to process service data');
  }
};
